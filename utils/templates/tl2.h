...
/* Below macros are used for multi-compiler support for unrolling loops */
...
inline int32_t three_lut_ctor(int8_t* qlut, bitnet_float_type* b, bitnet_float_type* lut_scales) {
    ...
    for (int k = 0; k < act_k / 24; ++k) {
        ...
        {%- for i in range(8) %}
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(qlut_i8 + k * 256 + {{ i }} * 32 + 0), ix[{{ i }}]);
        {%- endfor %}
    }
